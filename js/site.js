var config = {
    ...
    whereFieldName: "Admin2_pcode",
    sum: true,
    sumField: "beneficiaries",
    geo: "data/niger-adm2.json",
    joinAttribute: "admin2Pcod",
    nameAttribute: "admin2Name",
    color: "#3B88C0",
...
function generate3WComponent(config, data, geom) {
    ...
    var gp = cf.groupAll().reduce(
        function (p, v) {
            ...
            p.amountTransfered += +v["Total amount"];
            p.totalHH += +v["Households"];

            if (v["Name"] in p.orgas)
                p.orgas[v["Name"]]++;
            else {
                p.orgas[v["Name"]] = 1;
                p.numOrgs++;
